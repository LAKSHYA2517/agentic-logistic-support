...
The adapter reads provider settings from the shared application
configuration and remains independent of database persistence.
...
class IndicOCRProvider(HttpClientLifecycleMixin):
    ...
    ``AppSettings.indicocr_api_url``. The request is a
    ...
    def __init__(
        ...
        *,
        ...
        self._api_url = api_url or settings.indicocr_api_url
        if not self._api_url:
            raise OcrConfigurationError(
                "INDICOCR_API_URL environment variable is not set"
            )
        self._api_key = api_key if api_key is not None else settings.indicocr_api_key
